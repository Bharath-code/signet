...
export async function scrapeSite(url: string): Promise<{ html: string; screenshotUrl: string; finalUrl: string }> {
  ...
  return { html: html.slice(0, 20000), screenshotUrl, finalUrl };
}
