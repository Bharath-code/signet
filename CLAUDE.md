...
- **Brand kit is deterministic-first; Gemini is a gap-filler.** `extractBrandKit` builds the visual kit from Firecrawl's native `branding` format (`lib/brand-from-firecrawl.ts`) → CSS tokens → scrape meta. If logo + 2 colors + font + company name are all present it returns `source: 'firecrawl'` and **skips the Gemini vision call entirely**. Vision runs only to fill gaps (and to extract contact name/role/email/phone), returning `source: 'vision'`. Firecrawl/CSS values always override the model's. No durable cache yet (Firecrawl `maxAge` + the route's in-process Map only; JSON-mode contact data rides on the same scrape, so there is exactly one Firecrawl call per uncached request) — add KV when traffic justifies.
